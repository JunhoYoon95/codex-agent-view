...
Phase 0의 repository 조사와 구현 입력 정리는 완료됐다. Public npm `latest`/version은 `0.5.6`이며 accepted/registry tarball byte identity, registry digest/signature, main/tag CI, annotated tag와 public GitHub Release를 확인했다. Accepted exact artifact의 clean temporary actual CLI first install/uninstall과 exact extracted artifact browser E2E도 완료했다. 공식 Codex 앱 restart/trust/new-task real-hook status-filter/assignment/invocation E2E와 clean second-device install은 pending이다. Public `0.5.5` release evidence와 다른 기기의 actual not-found compatibility failure는 historical fact로 보존한다. Historical `0.2.1` 공식 앱 E2E에서는 task ID 등록 없이 parent 3개와 subagent 3개 자동 표시와 실제 hook 8종을 확인했다. `0.3.0` E2E에서는 공식 앱 내장 thread tools로 `kyurasi-next-supabase`의 explicit `inProgress` snapshot과 직후 `idle + hasUnreadTurn` 전환을 확인했으며, 후자는 별도 확인 대기 표시의 근거이지 완료·성공 추론의 근거가 아니다. Browser monitor에서는 실제 `SessionEnd` lifecycle 반영을 관찰했다. Historical public evidence는 아래에 보존한다.

Public npm `0.5.6`은 `2026-08-08T23:51:45.221Z`에 publish됐고 `latest`도 `0.5.6`이다. Registry tarball은 accepted exact tarball과 byte-identical하다. SHA-256은 `1833ab20f694c6ee10c16c76af19e968074c566ae39ffa53b88efee81b70de86`, npm shasum은 `9879c9fb98ee9abe768ca05e0c7dac1729015867`, integrity는 `sha512-0BIt98xDH1eDvqbI7OeGzwGcwv5YxanwJJ3YeduqN6ubiZiHW3M3x6iuql8LUAKs4k8THXf7YeE6L23bfvROkQ==`이며 signature, 23 files, packed `65770` bytes, unpacked `262872` bytes를 확인했다. Empty temporary `CODEX_HOME`/runtime과 actual `codex-cli 0.146.0`에서 exact tgz install 뒤 plugin `0.5.6` installed/enabled, copied catalog source `./plugins/codex-agent-view`, nested manifest only와 valid hook wiring 9종을 확인했다. Persisted trust는 CLI-unobservable `unknown`이었고 `uninstall --purge` 뒤 plugin/runtime은 absent였다.

Exact extracted artifact browser E2E에서는 zero-event Hook 검토 callout/4 steps와 viewer self-exclusion mutual exclusion을 확인했고 console warning/error는 0건이었다. Release commit과 annotated tag target `651d260c17694814a118f84705b3f809b7daa1a5`, main CI `31284502077`, tag CI `31284958057`의 Node.js 18/20/22 성공을 확인했다. [Public non-draft/non-prerelease GitHub Release v0.5.6](https://github.com/JunhoYoon95/codex-agent-view/releases/tag/v0.5.6)는 `2026-08-08T23:53:33Z`에 공개됐다. 이 evidence는 공식 Codex 앱 actual hook delivery 또는 clean second-device install을 대체하지 않는다.

`v0.5.4..v0.5.5`의 `hooks/`, `scripts/`, `src/` diff는 0이므로 hook collection, transport, normalization과 in-memory state의 privacy 경계는 바뀌지 않았다. Public `0.5.5`의 prepublish exact-tarball lifecycle과 isolated browser filter E2E는 통과했지만, exact artifact identity 및 compatible-CLI reinstall과 마찬가지로 clean cross-device first install 또는 공식 Codex 앱 hook/plugin E2E를 대체하지 않는다.

npm `latest`/version `0.5.5`, publish time `2026-08-04T18:42:31.744Z`, shasum `cc04e6a10380a0b5b0f9b1df5af7b6ce8d85139a`, integrity `sha512-J10ch73w6bi6Q/R4A3nkjYVaV9/HTnYxuu1Znmnin+xi432IJVgWlKyEPmPfdBlW+duoGyLzpV+mmfHN4DZydA==`와 signature 1개를 확인했다. Artifact는 23 files, packed `63.2 kB`, unpacked `253525 bytes`, tarball SHA-256 `242b007c8572474712d7694553e5319e193804ebc268c3d15008510258cc03d5`이며 local release와 clean-cache registry tarball은 byte-identical하다.

Release source `0037a5b304dc76a20050d2cf5dfce3276dccd004`의 main CI `30937777321`과 tag CI `30940008363`은 Node.js 18/20/22에서 성공했다. Annotated `v0.5.5`는 release source를 가리키고 [public non-draft/non-prerelease GitHub Release](https://github.com/JunhoYoon95/codex-agent-view/releases/tag/v0.5.5)는 `2026-08-04T18:44:32Z`에 공개됐다. This-device global package와 CLI `0.5.5`, plugin installed/enabled, valid hooks 9종, registry/global diff 0, registry install entries 13개와 marketplace diff 0을 compatible Codex CLI의 reinstall로 확인했다. Reinstall 뒤 monitor는 stopped이고 hook trust는 CLI-unobservable `unknown`이다. 이 결과는 clean first-install evidence가 아니다. 공식 Codex 앱 restart/new-task public exact status-filter/assignment E2E는 pending이다. Public `0.5.5`의 `source.path: "./"`를 지원하지 않는 Codex CLI가 선택된 다른 기기에서는 “plugin `codex-agent-view` was not found in marketplace `codex-agent-view`” 실패가 실제 확인됐다. `codex --version` 확인과 현재 CLI 업데이트는 임시 recovery이며 installer fix가 아니다. Universal Directory와 promptless plugin-card Quick start는 계속 pending/unverified다.
