...
Phase 0의 repository 조사와 구현 입력 정리는 완료됐다. Repository/package와 public npm `latest`/version은 `0.4.8`이다. Historical `0.2.1` 공식 앱 E2E에서는 task ID 등록 없이 parent 3개와 subagent 3개 자동 표시와 실제 hook 8종을 확인했다. `0.3.0` E2E에서는 공식 앱 내장 thread tools로 `kyurasi-next-supabase`의 explicit `inProgress` snapshot과 직후 `idle + hasUnreadTurn` 전환을 확인했으며, 후자는 별도 확인 대기 표시의 근거이지 완료·성공 추론의 근거가 아니다. Browser monitor에서는 실제 `SessionEnd` lifecycle 반영을 관찰했다. Historical public evidence는 아래에 보존하며 current public `0.4.8`의 source check, registry/artifact, this-device exact install, main/tag CI, annotated tag, GitHub Release, actual new subagent start/stop와 in-app Browser 인증/recovery 격리를 확인했다.
...
Public `0.4.8`의 정상 `$show-agents` 경로는 내부 `prepare-live-view` command 1회와 Codex in-app Browser open 요청 1회다. CLI는 fresh nonce/HMAC ownership proof로 exact owned monitor를 먼저 검증한 뒤 exact `127.0.0.1:<port>` authority의 origin-form request로만 runtime/control bearer를 보낸다. Runtime token이 서명한 1회용 60초 bootstrap grant만 Browser URL fragment에 넣고 persistent viewer/runtime token은 URL이나 일반 출력에 넣지 않는다. Bootstrap은 issuing process 안에서 1회만 쓸 수 있고 monitor restart 시 즉시 무효다. Validated `CODEX_THREAD_ID`는 signed family에 binding된다.
