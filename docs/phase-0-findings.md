...
Phase 0의 repository 조사와 구현 입력 정리는 완료됐다. Public npm `latest`/version은 `0.5.5`이며 historical registry/CI/tag/Release evidence와 다른 기기의 actual not-found compatibility failure를 보존한다. Current source는 copied marketplace strict-subdir installer correction을 포함한 unpublished `0.5.6` release candidate다. Accepted exact tarball identity, empty temporary `CODEX_HOME`/runtime의 actual CLI first install/uninstall과 exact extracted artifact browser E2E는 확인했다. 공식 Codex 앱 restart/trust/new-task status-filter/assignment/invocation E2E와 publish/CI/tag/Release는 pending이다. Historical `0.2.1` 공식 앱 E2E에서는 task ID 등록 없이 parent 3개와 subagent 3개 자동 표시와 실제 hook 8종을 확인했다. `0.3.0` E2E에서는 공식 앱 내장 thread tools로 `kyurasi-next-supabase`의 explicit `inProgress` snapshot과 직후 `idle + hasUnreadTurn` 전환을 확인했으며, 후자는 별도 확인 대기 표시의 근거이지 완료·성공 추론의 근거가 아니다. Browser monitor에서는 실제 `SessionEnd` lifecycle 반영을 관찰했다. Historical public evidence는 아래에 보존한다.

Accepted `0.5.6` exact tarball의 SHA-256은 `1833ab20f694c6ee10c16c76af19e968074c566ae39ffa53b88efee81b70de86`, npm shasum은 `9879c9fb98ee9abe768ca05e0c7dac1729015867`, integrity는 `sha512-0BIt98xDH1eDvqbI7OeGzwGcwv5YxanwJJ3YeduqN6ubiZiHW3M3x6iuql8LUAKs4k8THXf7YeE6L23bfvROkQ==`이며 23 files, packed `65770` bytes, unpacked `262872` bytes다. Empty temporary `CODEX_HOME`/runtime과 actual `codex-cli 0.146.0`에서 exact tgz install 뒤 plugin `0.5.6` installed/enabled, copied catalog source `./plugins/codex-agent-view`, nested manifest only와 valid hook wiring 9종을 확인했다. Persisted trust는 CLI-unobservable `unknown`이었고 `uninstall --purge` 뒤 plugin/runtime은 absent였다.

Exact extracted artifact browser E2E에서는 zero-event Hook 검토 callout/4 steps와 viewer self-exclusion mutual exclusion을 확인했고 console warning/error는 0건이었다. GitHub auth와 npm `kyurasi` auth/2FA `auth-and-writes`도 확인했다. 이는 npm publication, registry identity, CI/tag/Release 또는 공식 Codex 앱 actual hook delivery evidence가 아니다.
