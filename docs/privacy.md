...
Codex Agent View is an unofficial, read-only companion plugin for Codex. Its live monitor is local-only and communicates over IPv4 loopback (`127.0.0.1`). Public `0.5.6` opens that local page in the operating system's default browser when the user invokes `@codex-agent-view`; it does not send the page to a hosted service. The plugin-level starter prompt is empty, so the supported contract is selecting and sending `@codex-agent-view` itself and letting the internal single skill run `open`. Whether the Codex app shows a promptless plugin-card Quick start control is app UI behavior and is not promised without direct observation. This UI metadata does not alter the data flow described below. The project has no external telemetry, hosted service, cloud account, analytics SDK, remote database, or SQLite event store. Verified public `0.5.6` artifact identity, accepted/registry tarball byte identity, CI, tag and Release evidence is recorded in [Distribution](distribution.md). The accepted artifact passed an empty temporary `CODEX_HOME` actual-CLI install, but official-app real-hook E2E and clean second-device install remain pending. Public `0.5.5` clean cross-device first install was not verified, and its actual marketplace-root not-found failure is preserved as historical evidence.
