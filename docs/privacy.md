...
Codex Agent View is an unofficial, read-only companion plugin for Codex. Its live monitor is local-only and communicates over IPv4 loopback (`127.0.0.1`). Public package `0.5.3` opens that local page in the operating system's default browser when the user invokes `@codex-agent-view`; it does not send the page to a hosted service. The plugin-level starter prompt is empty, so the supported contract is selecting and sending `@codex-agent-view` itself and letting the internal single skill run `open`. Whether the Codex app shows a promptless plugin-card Quick start control is app UI behavior and is not promised without direct observation. This UI metadata does not alter the data flow described below. The project has no external telemetry, hosted service, cloud account, analytics SDK, remote database, or SQLite event store. Verified npm registry evidence for public `0.5.3` and historical `0.5.2` remains in [Distribution](distribution.md).

After the authenticated view is open, its periodic refresh reads local in-memory hook state and creates no additional ongoing model or external API inference calls for monitoring. The initial `@codex-agent-view` invocation is still a normal Codex turn and can use tokens, and the observed Codex tasks and subagents continue their normal token usage. The project therefore does not claim zero token usage or broadly claim no ongoing token usage for the complete user flow.
