...
Codex Agent View is an unofficial, read-only companion plugin for Codex. Its live monitor is local-only and communicates over IPv4 loopback (`127.0.0.1`). Current source targets the unpublished `0.5.6` release candidate and opens that local page in the operating system's default browser when the user invokes `@codex-agent-view`; it does not send the page to a hosted service. The plugin-level starter prompt is empty, so the supported contract is selecting and sending `@codex-agent-view` itself and letting the internal single skill run `open`. Whether the Codex app shows a promptless plugin-card Quick start control is app UI behavior and is not promised without direct observation. This UI metadata does not alter the data flow described below. The project has no external telemetry, hosted service, cloud account, analytics SDK, remote database, or SQLite event store. Verified public `0.5.5` artifact identity, this-device compatible-CLI reinstall, CI, tag and Release evidence is recorded in [Distribution](distribution.md). Clean cross-device first install was not verified, and an actual not-found failure is recorded for a Codex CLI that does not support the public package's `source.path: "./"` marketplace-root plugin. The `0.5.6` candidate contains a copied-marketplace strict-subdirectory correction, but its exact tarball clean-device install, official-app E2E, publication, CI, tag, and Release remain unverified or pending.

Open each view with one lightweight `@codex-agent-view` invocation. Once open, live monitoring runs locally with no additional model calls. The monitored Codex tasks and subagents continue their normal model and token usage; this project does not guarantee the opening invocation's exact token amount.
...
A verified `SubagentStart` does not include a dedicated assignment description or an exact correlation ID for the pending spawn candidate. The monitor therefore uses only a bounded best-effort correlation: a summary derived from a verifiable plaintext request or a safely humanized spawn task label can become an agent's one-line `assignment_summary` only when exactly one pending candidate and one newly observed agent remain in the unexpired bounded window. In the current official-app wire observation, the spawn `message` was an opaque protected `gAAAA...` value, so the humanized task label is the primary usable source. Opaque or protected values are rejected rather than displayed. Concurrent, ambiguous, or expired candidates produce no assignment summary. The raw spawn message is discarded during normalization, and no full prompt or tool input/output is added to transport, state, or UI. The displayed summary describes the assigned request or label, not internal reasoning or a continuously inferred plan.
