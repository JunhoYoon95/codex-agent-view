...
- 현재 단계: public `0.5.5` npm artifact/source-pack identity, main·tag CI, annotated tag/GitHub Release 및 이 기기의 compatible Codex CLI reinstall을 확인했지만 clean cross-device first install evidence는 아니며, 다른 기기의 root-path catalog 미지원 Codex CLI에서 actual plugin not-found 실패를 확인했다. Current source는 `0.5.6` release candidate로 copied marketplace strict-subdir installer fix를 포함하며 accepted exact tarball의 SHA-256 `1833ab20f694c6ee10c16c76af19e968074c566ae39ffa53b88efee81b70de86`, npm shasum/integrity와 23-file package metadata를 고정했다. Empty temporary `CODEX_HOME`/runtime과 actual `codex-cli 0.146.0`에서 exact tgz first install, plugin `0.5.6` installed/enabled, copied catalog `./plugins/codex-agent-view`, nested manifest only, valid hook wiring 9종과 uninstall `--purge` 뒤 plugin/runtime absent를 확인했다. Persisted hook trust는 CLI-unobservable `unknown`이다. Exact extracted artifact의 isolated browser E2E에서 zero-event hook callout/4 steps, viewer self-exclusion mutual exclusion과 console warning/error 0을 확인했다. GitHub auth와 npm `kyurasi` auth/2FA `auth-and-writes`도 확인했다. 아직 npm publish, main/tag CI, annotated tag/GitHub Release와 공식 Codex 앱 restart/trust/new-task actual hook/status-filter/assignment/invocation E2E는 pending이며 public `0.5.5`가 수정됐다고 표현하지 않는다. Assignment의 bounded/best-effort singleton correlation과 concurrent ambiguity fail-closed 경계를 유지한다. Public `0.5.5` evidence는 historical fact로 보존한다. Universal Plugins Directory validator·portal/reviewer·검색 노출과 promptless plugin-card Quick start는 별도 미확인 상태로 유지한다.
- `0.5.6` release candidate는 아직 publish/tag되지 않았으며 위 local artifact·CLI·browser evidence를 public release evidence로 승격하지 않는다.
