...
- 현재 단계: public npm `latest`/version은 `0.5.6`이다. Registry tarball은 accepted tarball과 byte-identical하고 SHA-256 `1833ab20f694c6ee10c16c76af19e968074c566ae39ffa53b88efee81b70de86`, npm shasum `9879c9fb98ee9abe768ca05e0c7dac1729015867`, integrity `sha512-0BIt98xDH1eDvqbI7OeGzwGcwv5YxanwJJ3YeduqN6ubiZiHW3M3x6iuql8LUAKs4k8THXf7YeE6L23bfvROkQ==`, signature와 23-file artifact를 확인했다. Release commit과 annotated `v0.5.6` target은 `651d260c17694814a118f84705b3f809b7daa1a5`이고 main CI `31284502077`과 tag CI `31284958057`은 Node.js 18/20/22에서 성공했으며 public non-draft/non-prerelease GitHub Release도 확인했다. Accepted artifact는 empty temporary `CODEX_HOME`/runtime과 actual `codex-cli 0.146.0`에서 first install·purge를 통과했고 plugin `0.5.6` installed/enabled, copied catalog `./plugins/codex-agent-view`, nested manifest only, valid hook wiring 9종과 trust `unknown`을 확인했다. 공식 Codex 앱 restart/trust/new-task real-hook E2E와 clean second-device install은 pending이다. Assignment의 bounded/best-effort singleton correlation과 concurrent ambiguity fail-closed 경계를 유지한다. Public `0.5.5`의 root-path actual plugin not-found failure와 release evidence는 historical fact로 보존한다. Universal Plugins Directory validator·portal/reviewer·검색 노출과 promptless plugin-card Quick start는 별도 미확인 상태로 유지한다.
